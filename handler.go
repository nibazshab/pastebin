...
func upload_data(w http.ResponseWriter, r *http.Request) string {
    ...
    if !regexp.MustCompile("^(text|image)/.*").MatchString(http.DetectContentType(con)) {
    ...
    db.Exec("INSERT INTO pastebin_data (id, data) VALUES (?, ?)", id, con)
...
func show_data(w http.ResponseWriter, r *http.Request) {
    ...
    err := db.QueryRow("SELECT data FROM pastebin_data WHERE id = ?", strings.TrimPrefix(r.URL.Path, "/")).Scan(&con)
    if err != nil {
